	...
	<div class="check-list list-simple">
		...
		?>
			<label class="item-label" for="<?php $site->cms->sanitizeText($id, true); ?>">
				<input type="radio" name="fields[<?php $site->cms->sanitizeText($config->field, true); ?>]" value="<?php $site->cms->sanitizeText($value, true); ?>" <?php echo($value == $selection ? 'checked="checked"' : ''); ?> id="<?php $site->cms->sanitizeText($id, true); ?>">
